...
*/
    ...
    function nm_calc_data3(&$dia, &$mes, &$ano, $dias_calc = 0, $mes_calc = 0, $ano_calc = 0, $direcao = '+')
    {
		...
		if (0 < $mes_calc) {
			$date_calc = date('Y-m-d', strtotime($date_calc. " $direcao $mes_calc months"));
		}
		if (0 < $ano_calc) {
			$date_calc = date('Y-m-d', strtotime($date_calc. " $direcao $ano_calc years"));
		}
		$dia = date('d', strtotime($date_calc));
		$mes = date('m', strtotime($date_calc));
		$ano = date('Y', strtotime($date_calc));
    }
